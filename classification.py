...
def plot_confusion_matrix(cm, labels, cmap='Blues_r'):
...
if __name__ == "__main__":
    ...
    for i in range(len(input_train)):
        ...
        # Collect errors and accuracy for averaging
        all_errors.append(keep_error)
        all_accuracies.append(keep_accuracy)

    # Calculate average errors and accuracies
    avg_errors = np.mean(all_errors, axis=0)
    avg_accuracies = np.mean(all_accuracies, axis=0)
    
    # Plot average MSE and accuracy over all folds in a single graph
    fig, ax1 = plt.subplots(figsize=(12, 6))

    color = 'tab:red'
    ax1.set_xlabel('Epoch')
    ax1.set_ylabel('Average MSE', color=color)
    ax1.plot(avg_errors, label='Average MSE', color=color)
    ax1.tick_params(axis='y', labelcolor=color)

    ax2 = ax1.twinx()
    color = 'navy'
    ax2.set_ylabel('Average Accuracy (%)', color=color)
    ax2.plot(avg_accuracies, label='Average Accuracy', color=color)
    ax2.tick_params(axis='y', labelcolor=color)

    # Add average values in the graph
    avg_error = np.mean(avg_errors)
    avg_accuracy = np.mean(avg_accuracies)
    
    ax1.text(0.5, 0.2, f'Avg MSE: {avg_error:.4f}', transform=ax1.transAxes, 
             ha='center', va='center', color='tab:red', fontsize=10)
    ax2.text(0.5, 0.8, f'Avg Accuracy: {avg_accuracy:.2f}%', transform=ax2.transAxes, 
             ha='center', va='center', color='navy', fontsize=10)

    fig.tight_layout()
    plt.title('Average MSE and Accuracy vs. Epoch', fontsize=14)
    plt.show()
